...
export const SelectName = ({ activeGameId }: Player2Props) => {
  ...
  const joinPlayer = async () => {
    ...
    try {
      ...
      setCurrentGame({ alive: true, gameId: activeGameId, token: data.token, view: GameState.Loading, boats: [], myGrid: new Array(100).fill(0), enemyGrid: new Array(100).fill(0), yourTurn: false, myName:"", enemyName:"" });
      setLoading(false);
    } catch (err) {
      setLoading(false);
      console.log("err...", err);
